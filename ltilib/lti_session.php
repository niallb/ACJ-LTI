...
class ltiSession
{
    ...
    static function Create($secretManager, $params)
    {
        ...
        $secret = $secretManager->getSecret($params["oauth_consumer_key"]);
        $sig = ltiSession::getOAuthSignature($params, getRequestURL(), $_SERVER['REQUEST_METHOD'], $secret);
        if($sig == $oldsig)
        	return new ltiSession($params);
        else
        {
        	//echo "Sig: $sig<br/>Old: $oldsig</br>Secret: $secret<br/>";
        	return false;
        }
    }

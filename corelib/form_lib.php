...
function formStart($target=false, $method='POST')
{
    ...
    $out = '<form class="form-horizontal" action="'.$target.'" method="'.$method.'">';
    return $out;
}
...
function textInput($caption, $name, $value="", $validateMsgs=null, $width=40)
{
    ...
    if((is_array($validateMsgs))&&(array_key_exists($name, $validateMsgs)))
        $out .= " has-error";
    $out .= \">";
    $out .= "<label class=\"control-label col-sm-3\" for=\"$name\">$caption</label>";
    $out .= "<div class=\"col-sm-9\"><span class=\"forminput\"><input type=\"text\" class=\"form-control\" name=\"$name\" value=\"$value\" size=\"$width\" /></span>";
    if((is_array($validateMsgs))&&(array_key_exists($name, $validateMsgs)))
